...
- **视频** → 取首帧作为封面，截前 3 秒（不解码、流拷贝），封装成 Live 图
...
3. 在「长视频分段时长（秒）」里填每段的秒数（默认 3，可填 1~30）。**只有长视频才会被切成多段**；图片和短视频（时长 ≤ 设定值时长）仍只生成 1 张 Live 图。
4. 点 **② 选择图片 / 视频**（可一次多选，挑你要转的文件）。
5. 点 **③ 开始转换**。
6. 进度条走完，日志里会显示每个文件已保存到 `你选的目录/Live图/xxx.jpg`。
   - 长视频会被切成多段：文件名形如 `xxx_1.jpg`、`xxx_2.jpg`……每段都是一个独立 Live 图。
...
- **长视频自动分段**：在「长视频分段时长（秒）」里设置每段秒数（默认 3）。视频时长超过该值时，会按此长度切成多段，每段各生成一个 Live 图（文件名加 `_1`/`_2`…后缀）；时长不超过该值时只生成 1 张。
- 图片会生成约 2 秒的放大动效（忽略分段设置）；视频分段忽略分段设置中的“前 3 秒”限制，按你填的每段秒数切。
