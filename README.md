...
- **图片** → 生成 3 秒「手持漂移感」动态视频（整幅画面平移 + 微旋转，**不做变焦放大**；内置 5 种风格每次随机取一种），再封装成 Live 图
...
- **图片生成约 3 秒「手持漂移感」动效**（两种模式都适用，不受分段设置影响）：
  ...
  - 改为模拟手持拍摄：整幅画面平移（横向 1.3%~3.0%、纵向 1.2%~2.1% 画幅）+ 微旋转（0.10°~0.32°）；
  - 运动曲线由两组不同频率的正弦叠加而成，有机、非匀速，避免机械的"推拉"感；
  - 首帧与末帧回到原始取景，因此**封面与你的原图完全一致**，播放时也没有跳变；
  - 裁切量按每帧位移动态计算（最小安定裁切），既不会露出黑边，也把画幅损失压到最低。
- **内置 5 种手持风格，每次随机取一种**（并在同风格内随机化运动相位），所以同一批图片处理出来各有各的动感，不会"一个模板套到底"：

  | 风格 | 特点 |
  |---|---|
  | 自然手持 | 中速随手漂移，横纵都有一点、旋转轻微 —— 最接近日常随手拍的实况图 |
  | 呼吸微推 | 慢速上下浮沉 + 极缓慢的轻微推近 —— 像拍摄者在轻轻呼吸 |
  | 漫步横移 | 慢速、以横向为主，纵向很弱 —— 像边走边拍，画面缓缓横向掠过 |
  | 轻快微抖 | 频率偏高、旋转偏多 —— 像手不太稳的随手一拍，抖动感明显 |
  | 斜向摇镜 | 横纵同频同相 —— 画面斜向缓缓摇过 |

  转换时日志会显示**本次实际用了哪种风格**（例如 `✓ 本次动效风格：漫步横移`）。
