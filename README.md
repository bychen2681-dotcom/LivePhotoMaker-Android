...
- 输出文件统一保存到：**手机存储 / Download / Live图** 文件夹下，文件名与原文件同名、扩展名 `.jpg`
...
> 已针对 Android 11+ 作用域存储做了处理，通过系统文件选择器读取、通过 MediaStore 写入 `Download/Live图`，**无需任何存储权限弹窗**。
...
1. 打开「Live图生成器」。
2. 点 **① 选择图片 / 视频**（可一次多选，挑你要转的文件）。
3. 点 **② 开始转换**。
4. 进度条走完，日志里会显示每个文件已保存到 `Download/Live图/xxx.jpg`。
5. 打开小米相册 / Google 相册，进入 **Download / Live图**，长按照片即可看到动态效果。

---

## 四、注意事项 / 排错

- **视频必须是 H.264 编码的 MP4**（手机拍的视频基本都是）。如果是 MOV / MKV / HEVC(H.265) 等，
  应用会提示「格式不支持」，需先在电脑上转成 H.264 MP4 再传入手机。
- 图片会生成约 2 秒的放大动效；视频只保留前 3 秒动效（与原脚本一致）。
- 个别国产相册可能不识别 Motion Photo，请用**小米相册**或 **Google 相册**查看动态效果；
  在文件管理器里它显示为一个普通的 `.jpg`，这是正常的。
- 若 `Download/Live图` 在某些机型上没自动建出来，文件会落在 `Download` 根目录，手动建一个 `Live图` 文件夹即可（不影响结果）。
